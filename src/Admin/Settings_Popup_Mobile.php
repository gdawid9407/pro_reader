...
class Settings_Popup_Mobile
{
    ...
    {
        $all_options = get_option(self::OPTION_NAME, []);
        $this->options = $all_options['mobile'] ?? [];
    ...
    public function page_init(): void
    {
        ...
        add_settings_section('popup_dimensions_section', __('Wymiary Popupa (Mobilny)', 'pro_reader'), null, $page);
    ...
    public function sanitize(array $input, array $current_options): array
    {
        ...
        $mobile_options['popup_max_width']                 = isset($input['popup_max_width']) ? absint($input['popup_max_width']) : 360;
        $mobile_options['popup_max_height']                = isset($input['popup_max_height']) ? absint($input['popup_max_height']) : 85;
        $mobile_options['popup_padding_y_desktop']         = isset($input['popup_padding_y_desktop']) ? absint($input['popup_padding_y_desktop']) : 20;
        $mobile_options['popup_padding_x_desktop']         = isset($input['popup_padding_x_desktop']) ? absint($input['popup_padding_x_desktop']) : 20;
        $mobile_options['popup_margin_content_bottom']     = isset($input['popup_margin_content_bottom']) ? absint($input['popup_margin_content_bottom']) : 15;
        $mobile_options['popup_gap_list_items']            = isset($input['popup_gap_list_items']) ? absint($input['popup_gap_list_items']) : 30;
        $mobile_options['popup_gap_grid_items']            = isset($input['popup_gap_grid_items']) ? absint($input['popup_gap_grid_items']) : 20;
        $mobile_options['popup_grid_item_width']           = isset($input['popup_grid_item_width']) ? absint($input['popup_grid_item_width']) : 150;
        
        $mobile_options['popup_rec_thumb_margin_right']    = isset($input['popup_rec_thumb_margin_right']) ? absint($input['popup_rec_thumb_margin_right']) : 15;
        $mobile_options['popup_rec_thumb_width_horizontal'] = isset($input['popup_rec_thumb_width_horizontal']) ? absint($input['popup_rec_thumb_width_horizontal']) : 120;
        $mobile_options['popup_rec_thumb_width_list_vertical'] = isset($input['popup_rec_thumb_width_list_vertical']) ? absint($input['popup_rec_thumb_width_list_vertical']) : 100;
        
        $mobile_options['popup_rec_margin_meta_bottom']    = isset($input['popup_rec_margin_meta_bottom']) ? absint($input['popup_rec_margin_meta_bottom']) : 5;
        $mobile_options['popup_rec_margin_title_bottom']   = isset($input['popup_rec_margin_title_bottom']) ? absint($input['popup_rec_margin_title_bottom']) : 8;
        $mobile_options['popup_rec_margin_excerpt_bottom'] = isset($input['popup_rec_margin_excerpt_bottom']) ? absint($input['popup_rec_margin_excerpt_bottom']) : 8;

        $mobile_options['popup_recommendations_layout']    = isset($input['popup_recommendations_layout']) && in_array($input['popup_recommendations_layout'], ['list', 'grid']) ? $input['popup_recommendations_layout'] : 'list';
        $mobile_options['popup_rec_item_layout']           = isset($input['popup_rec_item_layout']) && in_array($input['popup_rec_item_layout'], ['vertical', 'horizontal']) ? $input['popup_rec_item_layout'] : 'horizontal';
        $mobile_options['popup_rec_excerpt_limit_type']    = isset($input['popup_rec_excerpt_limit_type']) && in_array($input['popup_rec_excerpt_limit_type'], ['words', 'lines']) ? $input['popup_rec_excerpt_limit_type'] : 'words';
        $mobile_options['popup_rec_excerpt_length']        = isset($input['popup_rec_excerpt_length']) ? absint($input['popup_rec_excerpt_length']) : 10;
        $mobile_options['popup_rec_excerpt_lines']         = isset($input['popup_rec_excerpt_lines']) ? absint($input['popup_rec_excerpt_lines']) : 2;
        
        $allowed_components = ['thumbnail', 'meta', 'title', 'excerpt', 'link'];
        $mobile_options['popup_rec_components_order']      = $this->sanitize_order_array($input['popup_rec_components_order'] ?? [], $allowed_components);
        $mobile_options['popup_rec_components_visibility'] = $this->sanitize_visibility_array($input['popup_rec_components_visibility'] ?? [], $allowed_components);

        $allowed_sizes = array_keys($this->get_image_sizes_for_select());
        $mobile_options['popup_rec_thumb_size'] = isset($input['popup_rec_thumb_size']) && in_array($input['popup_rec_thumb_size'], $allowed_sizes) ? $input['popup_rec_thumb_size'] : 'thumbnail';
        
        $allowed_ratios = ['16:9', '4:3', '1:1', '3:4', 'auto'];
        $mobile_options['popup_rec_thumb_aspect_ratio'] = isset($input['popup_rec_thumb_aspect_ratio']) && in_array($input['popup_rec_thumb_aspect_ratio'], $allowed_ratios) ? $input['popup_rec_thumb_aspect_ratio'] : '4:3';

        $allowed_fits = ['cover', 'contain'];
        $mobile_options['popup_rec_thumb_fit'] = isset($input['popup_rec_thumb_fit']) && in_array($input['popup_rec_thumb_fit'], $allowed_fits) ? $input['popup_rec_thumb_fit'] : 'cover';
    ...
    {
        $value = $this->options['popup_max_width'] ?? 360;
        printf('<input type="number" id="popup_max_width" name="%s[mobile][popup_max_width]" value="%d" min="280" max="600" style="width: 100px;" />', self::OPTION_NAME, esc_attr($value));
    }

    public function max_height_callback(): void
    {
        $value = $this->options['popup_max_height'] ?? 85;
        printf('<input type="number" id="popup_max_height" name="%s[mobile][popup_max_height]" value="%d" min="50" max="100" style="width: 100px;" />', self::OPTION_NAME, esc_attr($value));
    }

    public function padding_y_desktop_callback(): void
    {
        $value = $this->options['popup_padding_y_desktop'] ?? 20;
        printf('<input type="number" id="popup_padding_y_desktop" name="%s[mobile][popup_padding_y_desktop]" value="%d" min="0" max="100" />', self::OPTION_NAME, esc_attr($value));
    }

    public function padding_x_desktop_callback(): void
    {
        $value = $this->options['popup_padding_x_desktop'] ?? 20;
        printf('<input type="number" id="popup_padding_x_desktop" name="%s[mobile][popup_padding_x_desktop]" value="%d" min="0" max="100" />', self::OPTION_NAME, esc_attr($value));
    }

    public function margin_content_bottom_callback(): void
    {
        $value = $this->options['popup_margin_content_bottom'] ?? 15;
        printf('<input type="number" id="popup_margin_content_bottom" name="%s[mobile][popup_margin_content_bottom]" value="%d" min="0" max="100" />', self::OPTION_NAME, esc_attr($value));
    }

    public function gap_list_items_callback(): void
    {
        $value = $this->options['popup_gap_list_items'] ?? 30;
        printf('<input type="number" id="popup_gap_list_items" name="%s[mobile][popup_gap_list_items]" value="%d" min="0" max="100" />', self::OPTION_NAME, esc_attr($value));
    }

    public function gap_grid_items_callback(): void
    {
        $value = $this->options['popup_gap_grid_items'] ?? 20;
        printf('<input type="number" id="popup_gap_grid_items" name="%s[mobile][popup_gap_grid_items]" value="%d" min="0" max="100" />', self::OPTION_NAME, esc_attr($value));
    }

    public function grid_item_width_callback(): void
    {
        $value = $this->options['popup_grid_item_width'] ?? 150;
        printf('<input type="number" id="popup_grid_item_width" name="%s[mobile][popup_grid_item_width]" value="%d" min="100" max="300" />', self::OPTION_NAME, esc_attr($value));
    ...
    {
        $value = $this->options['popup_rec_thumb_margin_right'] ?? 15;
        printf('<input type="number" id="popup_rec_thumb_margin_right" name="%s[mobile][popup_rec_thumb_margin_right]" value="%d" min="0" max="50" />', self::OPTION_NAME, esc_attr($value));
    ...
    {
        $value = $this->options['popup_rec_thumb_width_horizontal'] ?? 120;
        printf('<input type="number" id="popup_rec_thumb_width_horizontal" name="%s[mobile][popup_rec_thumb_width_horizontal]" value="%d" min="50" max="200" />', self::OPTION_NAME, esc_attr($value));
    ...
    {
        $value = $this->options['popup_rec_thumb_width_list_vertical'] ?? 100;
        printf('<input type="number" id="popup_rec_thumb_width_list_vertical" name="%s[mobile][popup_rec_thumb_width_list_vertical]" value="%d" min="10" max="100" />', self::OPTION_NAME, esc_attr($value));
    ...
    {
        $value = $this->options['popup_rec_margin_meta_bottom'] ?? 5;
        printf('<input type="number" id="popup_rec_margin_meta_bottom" name="%s[mobile][popup_rec_margin_meta_bottom]" value="%d" min="0" max="30" />', self::OPTION_NAME, esc_attr($value));
    }

    public function margin_title_bottom_callback(): void
    {
        $value = $this->options['popup_rec_margin_title_bottom'] ?? 8;
        printf('<input type="number" id="popup_rec_margin_title_bottom" name="%s[mobile][popup_rec_margin_title_bottom]" value="%d" min="0" max="30" />', self::OPTION_NAME, esc_attr($value));
    }

    public function margin_excerpt_bottom_callback(): void
    {
        $value = $this->options['popup_rec_margin_excerpt_bottom'] ?? 8;
        printf('<input type="number" id="popup_rec_margin_excerpt_bottom" name="%s[mobile][popup_rec_margin_excerpt_bottom]" value="%d" min="0" max="30" />', self::OPTION_NAME, esc_attr($value));
    }
    ...
    public function recommendations_layout_callback(): void
    {
        ...
        echo '<select id="popup_recommendations_layout" name="' . self::OPTION_NAME . '[mobile][popup_recommendations_layout]">';
    ...
    {
        $layout = $this->options['popup_rec_item_layout'] ?? 'horizontal';
        ?>
        <fieldset>
            <label>
        ...
        foreach ($order as $key) {
            ...
            printf(
                '<li style="padding:8px 12px;border:1px solid #ddd;margin-bottom:5px;background:#f9f9f9;cursor:move;display:flex;align-items:center;justify-content:space-between;">
                    <div><input type="checkbox" id="v_%1$s" name="%2$s[mobile][popup_rec_components_visibility][%1$s]" value="1" %3$s><label for="v_%1$s" style="user-select:none;padding-left:4px;">%4$s</label></div>
    ...
    {
        $value = $this->options['popup_rec_excerpt_length'] ?? 10;
        printf('<input type="number" id="popup_rec_excerpt_length" name="%s[mobile][popup_rec_excerpt_length]" value="%d" min="0" />', self::OPTION_NAME, esc_attr($value));
    }
    
    public function excerpt_lines_callback(): void
    {
        $value = $this->options['popup_rec_excerpt_lines'] ?? 2;
        printf('<input type="number" id="popup_rec_excerpt_lines" name="%s[mobile][popup_rec_excerpt_lines]" value="%d" min="0" style="width: 80px;" />', self::OPTION_NAME, esc_attr($value));
    }
    
    public function thumb_size_callback(): void
    {
        $value = $this->options['popup_rec_thumb_size'] ?? 'thumbnail';
        $sizes = $this->get_image_sizes_for_select();
        echo '<select id="popup_rec_thumb_size" name="' . self::OPTION_NAME . '[mobile][popup_rec_thumb_size]">';
    ...
    {
        $value = $this->options['popup_rec_thumb_aspect_ratio'] ?? '4:3';
        $ratios = [
            '16:9'  => '16:9 (Szeroki)', '4:3' => '4:3 (Standardowy)', '1:1' => '1:1 (Kwadrat)',
            '3:4' => '3:4 (Portret)', 'auto' => 'Auto (Dopasuj do wysokości)'
        ];
        echo '<select id="popup_rec_thumb_aspect_ratio" name="' . self::OPTION_NAME . '[mobile][popup_rec_thumb_aspect_ratio]">';
    ...
    public function thumb_fit_callback(): void
    {
        ...
        echo '<select id="popup_rec_thumb_fit" name="' . self::OPTION_NAME . '[mobile][popup_rec_thumb_fit]">';
